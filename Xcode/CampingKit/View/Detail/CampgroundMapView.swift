...
struct CampgroundMapView: View {
    ...
    var body: some View {
        Map(
            ...
            annotationItems: [campground]
        ) { location in
            MapAnnotation(coordinate: .init(location: campground.location)) {
                CampgroundAnnotationView(campground: location)
            }
        }
...
struct CampgroundMapView_Preview: PreviewProvider {
